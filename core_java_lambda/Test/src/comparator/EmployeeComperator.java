...
public class EmployeeComperator {
	...
	public static void main(String[] args) {
		...
		Function<EmployeeComperator, String> sortByNae = emp -> emp.getName();
		
		List<EmployeeComperator> sortedList =list.stream()  // ist approach with class reference
				.sorted(Comparator.comparing(sortByNae)  
				.thenComparing(EmployeeComperator::getSalary, Comparator.reverseOrder()))			
				.collect(Collectors.toList());
		
        list.sort(Comparator
                .comparing((EmployeeComperator e) -> e.getName())   // second approach without reference
